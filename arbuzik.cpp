...
public:
	...
	void gen_key(string& gde_alph, string& kuda_sohr) override
	{
		...
		if (array_alph != alph.end())
		{
			...
			{
				string text = *it;
				alph_data_str.push_back(text[0]);
				it++;
			}
	...
	void Shifr(string& zam_key_path, string& gde_sohranit, string& chto_za_sh) override
	{
		...
		ofstream encr_file(gde_sohranit);
	...
	void decrypt(string& zam_key_path, string& gde_sohranit, string& chto_za_sh) override
	{
		{
			setlocale(LC_ALL, "ru");
		};
		...
		ofstream decr_file(gde_sohranit);
...
private:
	...
	string shifraciya(string str, json key)
	{
		...
		for (int i = 0; i < str.size() / block_size; i++)
		{
			...
			{
				sh_str.push_back(new_str[key.at("key").at("block_value").at(j)]);
			}
...
public:
...

	{
		...
		ofstream sh_file(where_to_save);
	...
	void decrypt(string& gamm_key_put, string& gde_sohranit, string& chto_za_ne)
	{
		fstream key_file(gamm_key_put);
		if (!key_file.is_open())
		{
