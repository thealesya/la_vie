...
public:
	...
	void gen_key(string& gde_alph, string& kuda_sohr) override 
		//ГЕНЕРАЦИЯ КЛЮЧА ПЕРЕСТАНОВКА есть два числа длина блока символьного и числа в этом блоке, массив, который по длине соответствует
		//и вот мы во второе значение блока генерируем рандомный элемент, которые не привышают длину
		// пользователь пишет длину блока например 10 и программа генерирует все числа от 1 до 0

	{
		...
		// вся сгенерированная инфа хранится в джейсоновской переменной кей, только внутри данной программы, а дальше эту переменную переписываю в файл
		cout << key;
		...
		if (array_alph != alph.end())
		{
			...
			{
				okno = rand() % alph_data_str.size(); //запихиваем рандомное число
				elem.push_back(alph_data_str[okno]);
				if (repeat_check(okno) == false) // провека на повторы, если введенное число соответствует тому что уже лежит в векторе на каком то месте, значит повтор и цикл работает заново
