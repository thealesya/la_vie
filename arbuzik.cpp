...
public:
	...
	void key_gen(string& path_to_alph, string& path_to_save)
	{
		...
		if (array_alph != alph.end())
		{
			...
			cout << "\n\nEnter a block length: ";
			cin >> block_length;//указывает пользователь
			if (block_length <= 0)
			{
				cout << "\n\nIncorrect block length!\n\n";
				exit(0);
		...
		if (x == 1)
		{
			...
			else if (x == 2)
			{
				...
				else if (x == 3)
				{
					...
					cout << "\n\nВведи путь к мусору в формате encrypt): "; string what_to_decrypt; cin >> what_to_decrypt;


					cout << "Теперь ключ от мусора в формате key): "; string key; cin >> key;
					if (key.substr(key.size() - 4, 4) != ".key")
					{
						cout << "\n\nВсё фигня, давай по новой\n\n";
						return 0;
